...
class DetailOrderNotifier extends AppProvider {
  ...
  _getBooking() async {
    ...
    if (response.success) {
      ...
      // Hentikan auto-refresh jika status sudah bukan FINDING_DRIVER
      if (_booking?.status == STATUS_FINDING_DRIVER) {
        hideLoading();
        await Future.delayed(const Duration(seconds: 10));
        if (!_isDisposed) _getBooking(); // Cek disposed sebelum refresh
      } else {
        hideLoading();
        if (_booking?.status == STATUS_DRIVER_PICKUP ||
            _booking?.status == STATUS_DRIVER_DELIVER) {
  ...
  updateStatus() async {
    ...
    try {
      ...
      if (response.success && !_isDisposed) {
        await _getBooking(); // Gunakan await untuk menghindari race condition
      } else {
        snackBarMessage = response.message;
      }
    } finally {
  ...
  cancel() async {
    ...
    try {
      ...
      if (_isDisposed) return; // Cek jika notifier sudah dispose

      if (response.success) {
        // Tidak perlu panggil init() lagi yang bisa menyebabkan loop
        // Cukup update data booking langsung
        final updatedBooking = await _bookingGetByIdUseCase(param: _id);
        if (updatedBooking.success && !_isDisposed) {
          _booking = updatedBooking.data;

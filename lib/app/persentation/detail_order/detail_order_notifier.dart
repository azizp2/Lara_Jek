...
class DetailOrderNotifier extends AppProvider {
  ...
  final MapController _mapController = MapController.cyclOSMLayer(
      initPosition: GeoPoint(latitude: 0.0, longitude: 0.0));
  MapController _mapFullScreenController = MapController.cyclOSMLayer(
      initPosition: GeoPoint(latitude: 0.0, longitude: 0.0));
  bool _isShowDialog = false;
  ...
  _getBooking() async {
    // showLoading();
    if (_isFirstLoad) {
      showLoading(); // Hanya tampilkan loading pada first load
    }
    ...
    if (response.success) {
      ...
      if (_booking?.status == STATUS_FINDING_DRIVER ||
          _booking?.status == STATUS_ARRIVED) {
        if (_isFirstLoad) {
          hideLoading(); // Sembunyikan loading setelah first load selesai
          _isFirstLoad = false; // Set flag first load ke false
        }
      ...
      errorMessage = response.message;
      if (_isFirstLoad) {
        hideLoading(); // Sembunyikan loading jika error pada first load
        _isFirstLoad = false;
      }
    }
